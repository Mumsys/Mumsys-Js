"use strict";class Mumsys_Exception extends Error{static getVersion(){return'3.0.0';}
constructor(message){super(message);this.name=this.constructor.name;if(typeof Error.captureStackTrace==='function'){Error.captureStackTrace(this,this.constructor);}else{this.stack=(new Error(message)).stack;}}}
...
class Mumsys_File_Item_Default{static getVersion(){return'3.0.0';}
...
extends Mumsys_Exception{static getVersion(){return'3.0.0';}}
class Mumsys_Generic_Item_Default{static getVersion(){return'3.1.0';}
constructor(params){this.__itemProps={};this.__m=false;if(params instanceof Object){this.__itemProps=params;if(undefined!==params.id){this.set('id',params.id);}}else{var message='Invalid parameters';throw new Mumsys_Generic_Exception(message);}
...
this.__m=_flag;}
_checkId(oldID,newID){if(newID!=undefined&&oldID!=undefined&&oldID!=newID){var message='New item ID "'+newID+'" differs from old ID "'+oldID+'"';throw new Mumsys_Generic_Exception(message);}
return newID;}}
...
createItem(props){if(props instanceof Object){return new Mumsys_Generic_Item_Default(props);}else{var message='Invalid properties';throw new Mumsys_Generic_Exception(message);}}
addItem(item){if(item instanceof Mumsys_Generic_Item_Default){var id=item.get('id');if(id!==undefined&&this.__map[id]!==undefined){var message='"id" ('+id+') is unique and already exists';throw new Mumsys_Generic_Exception(message);}
if(id!==undefined){this.__map[id]=this.__itemList.length;}
this.__itemList.push(item);}
else{throw new Mumsys_Generic_Exception('Invalid item');}}
...
saveItem(item,params,requestOptions=false){if(params.item!==undefined){var message='params.item property already defined';throw new Mumsys_Exception(message);}
...
this.__m=flag;};Mumsys_Generic_Item.prototype._checkId=function(oldID,newID){if(newID!=undefined&&oldID!=undefined&&oldID!=newID){var message='New item ID "'+newID+'" differs from old ID "'+oldID+'"';throw new Error(message);}
return newID;}
function Mumsys_Generic_Manager(url=false){this.__url='jsonrpc.php';if(url){this.__url=url;}
this.__itemList=[];this.__flags={"isLoaded":false};this.__map={};}
Mumsys_Generic_Manager.prototype.getVersion=function(){return'3.0.0';};Mumsys_Generic_Manager.prototype.createItem=function(props){if(props instanceof Object){return new Mumsys_Generic_Item(props);}else{var message='Invalid properties';throw new Error(message);}}
Mumsys_Generic_Manager.prototype.addItem=function(item){if(item instanceof Mumsys_Generic_Item){var id=item.get('id');if(id!==undefined&&this.__map[id]!==undefined){var message='"id" ('+id+') is unique and already exists';throw new Error(message);}
if(id!==undefined){this.__map[id]=this.__itemList.length;}
