...
return;}}
class Mumsys_Generic_Item_Default{static getVersion(){return'3.0.0';}
constructor(params){this.__itemProps={};this.__m=false;if(params instanceof Object){this.__itemProps=params;if(undefined!==params.id){this.set('id',params.id);}}else{var message='Invalid parameters';throw new Error(message);}
...
this.__m=_flag;}
_checkId(oldID,newID){if(newID!=undefined&&oldID!=undefined&&oldID!=newID){var message='New item ID "'+newID+'" differs from old ID "'+oldID+'"';throw new Error(message);}
return newID;}}
class Mumsys_Generic_Manager_Default{static getVersion(){return'3.0.0';}
constructor(url=false){this.__url='jsonrpc.php';if(url){this.__url=url;}
this.__itemList=[];this.__flags={"isLoaded":false};this.__map={};}
createItem(props){if(props instanceof Object){return new Mumsys_Generic_Item_Default(props);}else{var message='Invalid properties';throw new Error(message);}}
addItem(item){if(item instanceof Mumsys_Generic_Item_Default){var id=item.get('id');if(id!==undefined&&this.__map[id]!==undefined){var message='"id" ('+id+') is unique and already exists';throw new Error(message);}
...
saveItem(item,params,requestOptions=false){if(params.item!==undefined){var message='params.item property already defined';throw new Error(message);}
